...
CREATE TABLE public.virtual_mailbox_billing_config (
  ...
  late_fee_amount NUMERIC(10,2) NOT NULL DEFAULT 25.00,
...
-- Add cost_amount column to mail_actions table
...
CREATE TABLE public.virtual_mailbox_billing (
  ...
  virtual_mailbox_id UUID NOT NULL REFERENCES public.virtual_mailboxes(id),
...
-- Create policies
CREATE POLICY "Staff can manage billing records" ON public.virtual_mailbox_billing
  FOR ALL USING (auth.uid() IS NOT NULL);
